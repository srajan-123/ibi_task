...
public:
    ...
    std::optional<std::reference_wrapper<Employee>> findById(int employeeID);
...
std::optional<std::reference_wrapper<Employee>> EmployeeRepository::findById(int employeeID) {
    auto it = std::find_if(m_employees.begin(), m_employees.end(),
        [employeeID](const Employee& e) {
        return e.getEmployeeID() == employeeID;
    });
    ...
    if (it != m_employees.end()) {
        return std::ref(*it); // Return a reference wrapper to the found employee
    }
...
void handleUpdateEmployee(EmployeeRepository& repo) {
    ...
    auto employeeOpt = repo.findById(id);
    if (!employeeOpt) {
        std::cout << "Error: Employee with ID " << id << " not found." << std::endl;
        return;
    }
    
    Employee& emp = employeeOpt->get(); // Get the mutable employee reference
